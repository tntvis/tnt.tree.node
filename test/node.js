...
describe ('TnT node', function () {
    ...
    describe('API', function () {
        ...
        describe ('n_hidden', function () {
            ...
            it ("Returns the same as get_all_nodes with hidden branches set to true minus 1", function () {
                var root = tnt_node (newick.parse_newick (newickStr));
                var primates = root.find_node_by_name ('primates');
                primates.toggle();
                var n = primates.get_all_nodes().length;
                var n_hidden = primates.get_all_nodes(true).length;
                assert.strictEqual(primates.n_hidden(), n_hidden-1);
            });
        });
